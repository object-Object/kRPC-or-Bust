...
def vec_angle(v1, v2):
	return math.acos(clamp(vec_dot(v1, v2)/(vec_magnitude(v1)*vec_magnitude(v2)), -1, 1)) # clamp because of floating point errors
...
def hohmann_moon_rendezvous(conn, vessel, target, target_altitude, target_inclination=0, do_circularize=True):
	"""Vessel and target must be in circular coplanar orbits."""
	dv = _get_hohmann_dv(vessel.orbit, (target.orbit.apoapsis_altitude+target.orbit.periapsis_altitude)/2)
